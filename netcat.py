...
def execute(cmd):
    ...
    try:
        ...
        if current_os == "Windows":
            output = subprocess.check_output(f"cmd.exe /c {cmd}", stderr=subprocess.STDOUT, shell=True)
        else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="BHP net tool",formatter_class=argparse.RawDescriptionHelpFormatter,epilog=textwrap.dedent("""Example:
                               ...
                               netcat.py -t 192.168.1.108 -p 5555 -l -e=\"cat /etc/password\" #execute commnd
                               echo 'ABC' | ./netcat.py -t 192.168.0.108 -p 135 #echo text server port 135
